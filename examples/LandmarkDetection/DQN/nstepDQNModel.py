...
class Model3D(ModelDesc):
    ...
    def inputs(self):
        # Use a combined state for efficiency.
        # The first h channels are the current state, and the last h channels are the next state.
        # I'm not sure that if I leave out the look up dictionary from the InputSecc
        # if it will through an error or not.
        return [InputDesc(tf.uint8,
                          (None,) + self.image_shape + (self.channel + 1,),
                          'comb_state'),
                InputDesc(tf.int64, (None,), 'action'),
                InputDesc(tf.float32, (None,), 'reward'),
                InputDesc(tf.bool, (None,), 'isOver'),
                InputDesc(tf.int64, (None,), 'episode'),
                InputDesc(tf.int64, (None,), 'step_index'),
                InputDesc(tf.uint8,
                                  (None,) + self.image_shape + (self.channel + 1,),
                                  'ten_state'),
                InputDesc(tf.float32, (None,)+10, 'list_ten_rewards')]
    ...
    def build_graph(self, *inputs):
        comb_state, action, reward, isOver, episode, step_index, look_up_dic = inputs
        comb_state = tf.cast(comb_state, tf.float32)
